...
/**
 * Re-activate a unsuspended user
 * @method unsuspend
 * @param id
 * @param callback
 */
OktaAPIUsers.prototype.unsuspend = function(id, callback) {
    if(id == undefined) throw new Error("A user ID is required when unsuspending a user");
    this.request.post("/users/" + id + "/lifecycle/unsuspend", null, null, callback);
}
...
function mergeProfiles(what, userObj, updateProf, updateCreds, callback) {
    ...
    if(updateCreds != undefined) {
        finalCreds = {};
        for(let prop in userObj.credentials) finalCreds[prop] = userObj.profile[prop];
        for(let prop in updateCreds) finalCreds[prop] = updateCreds[prop];
    }
